...
import { GerBoardListResponseDto, GetSearchBoardListResponseDto } from "./dto/response";
...
export const getBoardListRequest = async (accessToken: string) => {
  const result = await axios.get(GET_BOARD_LIST_URL, bearerAuthorization(accessToken))
    .then(requestHandler<GerBoardListResponseDto>)
    .catch(requestErrorHandler);
  return result;
}

// function: Q&A 검색 리스트 불러오기 API 함수
export const getSearchBoardListRespuest = async (searchWord: string, accessToken: string) => {
