...
UPDATE jeju SET above_avg_spd = true WHERE speed_80m > 5.5;
UPDATE jeju SET above_avg_spd = false WHERE speed_80m <= 5.5;
...
-- 연산자
-- 자바에서 계산하는 것보다 SQL에서 먼저 계산하는 것이 빠름
-- 산술연산자: +, -, *, /, %
...
-- 논리연산자
-- AND(&&) : 좌항과 우항이 모두 true이면 true(교집합)
SELECT * FROM jeju WHERE speed_80m > 4 AND direction_50m < 180;
SELECT * FROM jeju WHERE speed_80m > 4 && direction_50m < 180;
-- OR(||) : 좌항과 우항 중 하나라도 true이면 true(합집합)
SELECT * FROM jeju WHERE speed_80m > 4 OR direction_50m < 180;
SELECT * FROM jeju WHERE speed_80m > 4 || direction_50m < 180;
-- XOR : 좌항과 우항이 다르면 true(교집합 제외)
...
-- 중복제거
-- DISTINCT: SELECT 결과 테이블에서 컬럼의 조합의 ㅈ중복을 제거하여 출력
