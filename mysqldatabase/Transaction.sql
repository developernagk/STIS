...
-- Consistency(일관성): 트랜잭션의 작업 결과는 항상 일관성이 있어야 함
-- Isolation(독립성): 트랜잭션은 모두 독립적이어야 함
-- Durablity(영구성): 트랜잭션이 성공적으로 완료되면 영구히 반영되어야 함

-- 회원가입: user - email_authentication
-- 사용자는 아이디를 입력하고 중복 확인을 함
-- 만약 입력한 아이디가 중복된 아이디가 아니라면 비밀번호, 비밀번호 확인, 이메일을 입력하고 이메일로 인증 번호를 전송
...
-- 사용자가 아이디를 'idddd'로 입력함
-- 1. 아이디 중복 확인
SELECT * FROM user WHERE id = 'idddd';

-- 사용자가 비밀번호를 'P!ssw0rd', 비밀번호 확인을 'P!ssw0rd', 이메일을 'email@email.com', 서버가 생성한 코드는 'OCMD'이다.
-- 2. 이메일 인증
INSERT email_authentication VALUES ('email@email.com', 'OCMD');

-- 서버가 지정한 이메일로 인증코드를 전송하고 사용자는 그 코드를 확인한 후 인증코드를 입력하여 인증함
-- 3. 이메일 인증 확인
SELECT * FROM email_authentication WHERE email = 'email@email.com' AND authentication_code = 'ZSDD'; # 인증코드 잘못 작성했을 경우
SELECT * FROM email_authentication WHERE email = 'email@email.com' AND authentication_code = 'OCMD';
...
SET AUTOCOMMIT = 0; # 커넥션 끊고 다시 접속하면 autocommit 끄기

-- 트랜잭션 시작
START TRANSACTION;

-- 사용자가 아이디를 'idddd3'로 입력함
-- 1. 아이디 중복 확인
SELECT * FROM user WHERE id = 'idddd3';

-- 사용자가 비밀번호를 'P!ssw0rd', 비밀번호 확인을 'P!ssw0rd', 이메일을 'email3@email.com', 서버가 생성한 코드는 'OCMD'이다.
-- 2. 이메일 인증
...
-- LIMIT 제외 레코드 개수1, 개수2 : 상위에서 제외 레코드 개수1만큼 제외 후 개수2만큼의 데이터만 보여줌
SELECT * FROM jeju LIMIT 10, 3; # 상위 11번째부터 3개