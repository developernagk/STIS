...
// 조건부 렌더링 :
// - 상황(조건)에 따라서 결과를 다르게 렌더링 하려 할 때 사용하는 방법
// - if, &&, ? :(삼항연산자) 을 사용하는 방법이 존재함
...
function IfComponent ({ number }: { number: number }) {
  ...
  // ex) number가 양수면 number값을 <h1>이면서 빨간색으로 표시, 양수가 아니면 number값을 <h3>이면서 파란색으로 표시
  // else는 생략해도 됨
  if (number > 0) return (
...
function AndComponent ({ number }: { number: number }) {
  // && 연산자를 이용한 조건부 렌더링
  // - JSX 내부(return 구문)에서 상황(조건)에 따라 렌더링 여부를 결정하고 싶을 때 사용
...
function ThreeTermComponent ({ number }: { number: number }) {
  ...
  // - JSX 내부에서(return 구문) 상황(조건)에 따라 서로 다른 렌더링을 하고 싶을 때 사용
