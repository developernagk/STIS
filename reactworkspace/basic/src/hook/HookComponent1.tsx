...
// - useReducer : 상태 로직을 관리하는 함수, 상태 업데이트 로직을 컴포넌트 외부로 옮길 수 있게 하여 코드 관리를 조금 더 수월하게 해줌
...
export default function HookComponent1() {
  ...
  // - 개발자 모드에서는 mount - unmount가 한 번 실행되면서 테스트 후 다시 정상 실행
  ...
  useEffect( () => {
    // 스코프할 상태 배열을 지정하지 않은 effect에서 상태를 변경하면 무한 실행 됨
    // setFlag(!flag);
  ...
  useEffect( () => {
    // 스코프할 상태 배열에 지정한 상태를 해당 effect에서 변경하면 무한 실행 됨
    // setCount(count + 1); -> <div>에서 {count}로 작성
