package com.nagyeong.basic.repository;
...
extends JpaRepository<StudentEntity, Integer> {
  ...
  // SQL : SELECT * FROM student WHERE address = '서울특별시';(0~여러 개를 조회하기 때문에 List<> 형태)
  ...
  // SQL : SELECT * FROM student WHERE student_number = 5 AND age > 20;(0~1개를 조회하기 때문에 List 형태가 필요 없음)
  ...
  // ex) SELECT student_number AS studentNumber,
  // name, age, address, graduation FROM student WHERE student_number = 5 AND age > 20;

  // JPQL(Java Persistence Query Language) :
  // - 표준 SQL과 매우 흡사하지만 Entity 명과 Entity 속성으로 쿼리를 작성하는 방법
  @Query(
    value="SELECT s FROM student s WHERE s.studentNumber = ?1 AND s.age > ?2",
    nativeQuery=false
  )
