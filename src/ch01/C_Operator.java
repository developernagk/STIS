...
public class C_Operator {
	...
	public static void main(String[] args) {
		...
		// 감소 연산자: --
		int number = 5;
		// 5를 더하기 전에 피연산자인 number의 값을 1 증가시킨 후 연산하기 때문에 출력 결과에 영향 있음
		intResult = ++number + 5; // 출력 결과 : 11, (증가연산 먼저 처리)5 + 1 = 6 -> 6 + 5 = 11
		// 먼저 number와 5를 더하고 값을 증가시키기 때문에 출력 결과에 영향 없음
		intResult = number++ + 5; // 출력 결과 : 10, (사칙연산 먼저 처리)5 + 5 = 10 -> (이후 증가연산)5 + 1 = 6
		System.out.println(intResult);
		System.out.println(number);
		
		// 대입(할당) 연산자: 좌항에 우항을 할당
		// 일반 대입 연산자: =
		// 상수에 초기화할 때를 제외하고 좌항이 반드시 변수이어야 함
		// 좌항에 우항을 할당
		number = 10;
		...
		number += 10;
		number -= 7;
		number *= (7 - 5); // 우항 먼저 계산 후 대입 연산
		System.out.println(number);
		// int number1;
		// number1 += 10; 초기화가 안 되어있기 때문에 컴파일에러 발생
		
		// 비교 연산: 좌항과 우항을 비교하여 그 결과를 논리값으로 반환, 연산자 중에서 가장 많이 사용
		...
		// a: 15, b: 7
		// 좌항이 true일 경우 우항까지 확인
		booleanResult = (a > b) && (a > ++b);
		System.out.println(booleanResult);
		System.out.println(b);
		
		// 좌항이 false일 경우 우항까지 확인
		booleanResult = (a < b) || (a > ++b);
		System.out.println(booleanResult);
		System.out.println(b);
